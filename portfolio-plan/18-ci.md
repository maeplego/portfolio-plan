...
| 項目 | 値 |
| --- | --- |
| 対象 | P01–P16 の製品リポジトリと、このメタリポジトリの書類 |
| 最終更新 | 2026-08-20 |
...
| 依存の脆弱性 | Node: `npm audit --audit-level=high`、または Next/Expo 推移的 HIGH を `scripts/npm-audit-allowlist.json` で理由付き除外。Go: `govulncheck`。Python: `pip-audit`。**Java（pf-attendance）は Trivy fs で依存 CVE をカバー**（OWASP dependency-check は必須にしない） | ジョブ失敗。`npm audit --force` は使わない |
