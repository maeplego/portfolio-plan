...
| 対象スライス | 受け入れは 1–5。6–7 は計画であり本書類の合格条件に含めない |
| 最終更新 | 2026-08-18 |
...
Wiki・チャット・共同編集のうち、スライス 3 はツリーと単一ユーザー Markdown まで。スライス 4 で Wiki 本文と独立ドキュメントに CRDT を接続する。スライス 5 はチャンネル履歴 REST と別 WS。
...
- チャンネルにメッセージを投稿でき、seq で順序と再接続差分が取れる
- typing は永続化しない。guest は履歴を読めて投稿できない

### 含まない（意図的。計画スライスでも「できた」扱いにしない）

| 項目 | 理由 |
| --- | --- |
| 全文検索 | スライス 6。権限外を出さないことはスライス 3 の GET/tree とチケット発行で先に守る |
