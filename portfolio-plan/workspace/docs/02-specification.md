...
| メンバー追加 | 可 | 否 | 否 | 否 |
| メンバー role 変更・除名 | 可 | 否 | 否 | 否 |
| ワークスペース退出（leave） | 可※ | 可 | 可 | 否 |
| ボード作成 | 可 | 可 | 否 | 否 |
| ボード参照 | 可 | 可 | 可 | 否 |
| カード作成・更新・移動 | 可 | 可 | 否 | 否 |
...
※ 唯一の owner は leave / demote / 除名できない（WS に owner が残ることを保証）。
...
- メンバー role 変更: owner のみ。`member` / `guest` のみ。監査 `workspace.member.role_updated`。
- 除名: owner のみ（自己除名は leave を使う）。監査 `workspace.member.removed`。
- 退出: 所属者が `POST .../leave`。監査 `workspace.member.left`。
...
- 列の追加・改名・削除・並べ替えは member 以上。空でない列と最後の 1 列は削除不可。
