...
| 対象スライス | o11y Compose、計装契約、連携 overlay A/B/C、Terraform モジュール（apply は非目標） |
| 最終更新 | 2026-08-19 |
...
| 7 Terraform モジュール + GitHub OIDC | 実装済み（モジュール） | 設計の 3-tier。apply は計画ではなく非目標 |
| 8 学習アカウントへ 3-tier 載せる | **非目標** | README のコスト・destroy のみ |
| 9 障害注入手順の文書化 | 計画 | o11y debug エンドポイントは実装済み。P12 題材は後続 |
