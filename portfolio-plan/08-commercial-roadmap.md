...
評価ライセンスのまま全製品を「商用フル」と呼ばない。ゲートは [09-production-definition.md](./09-production-definition.md)。パッケージ境界は [10-package-catalog.md](./10-package-catalog.md)。差し替えは [12-portability.md](./12-portability.md)。動作確認の層は [06-verification.md](./06-verification.md)。
...
flowchart LR
  ...
  P16[P16 payroll tax backoffice]
  M0 --> M1 --> M2 --> M3 --> P16
```

### M0 — 定義とカタログ（書類）

- [x] `09-production-definition.md`
- [x] `10-package-catalog.md`
- [x] `12-portability.md` / `13-portability-byo-idp.md`
- [x] 本ファイル
- [x] `16-cost-estimate.md` / `15-licensing.md` / `05-review.md` / `00-overview.md` からのリンク更新
...
- [x] BYO 手順ドラフト（`13-portability-byo-idp.md`）
- [x] P04 の org 隔離（memory でも tenant filter）+ `org_required` + BYO org 切替フォールバック（`rp_active_org` / `X-Workspace-Org`）
- [x] Collab staging 手順（`19-collab-staging.md`）と Collector down アラート骨格
...
- [x] 招待一枚絵・SLO／監査保持／脆弱性方針（[10-invite-visibility.md](./workspace/docs/10-invite-visibility.md)、[14-collab-slo-security.md](./14-collab-slo-security.md)）
...
- [x] Commerce staging 手順・overlay 入口（[20-commerce-staging.md](./20-commerce-staging.md)、`docker-desktop-d-commerce-staging`）
...
- [x] Media staging を Collab/Commerce staging overlay に載せる（[23-media-staging.md](./23-media-staging.md)）
...
- [x] Talent path staging overlay／手順（[21-talent-staging.md](./21-talent-staging.md)、`docker-desktop-c-scheduling-talent-staging`）
...
- [x] Attendance: staging overlay／手順（[22-attendance-staging.md](./22-attendance-staging.md)、`docker-desktop-f-ops-staging`）
...
- 棚卸し正本: [11-implementation-backlog-by-pxx.md](./11-implementation-backlog-by-pxx.md)

### P16 — 給与・税務／バックオフィス

**方針の正本**: [./payroll-platform/payroll-tax-policy.md](./payroll-platform/payroll-tax-policy.md)。製品 ID は **P16 payroll-platform**（`../pf-payroll`）。staging: [24-payroll-staging.md](./24-payroll-staging.md)。

**やる**: 給与連携 →（任意）薄いドメイン →（ゲート後のみ）源泉・年末調整等を名乗る。フル自前 ERP は非優先。

| 段階 | 内容 | 状態 |
| --- | --- | --- |
| 方針 | 方針・Ports・カタログ対象外の維持 | **完了**（2026-08-21） |
| Ports | P09 集計エクスポート + `PayrollExportPort` / `AccountingPort` モック | **完了**（`pf-payroll` + f-ops-staging） |
| デモ明細 | 薄いドメイン／明細プレビュー（disclaimer 常時） | **完了**（2026-08-21） |
| 規制名乗り | 規制名乗り（専門家レビュー後のみ） | **未着手** — 手順は [./payroll-platform/payroll-tax-policy.md](./payroll-platform/payroll-tax-policy.md) |

- 着手条件: M2 Go 済み、M3 主要パッケージ（Commerce / Talent / Attendance）の staging 入口済み
- ゲート: [./payroll-platform/payroll-tax-policy.md](./payroll-platform/payroll-tax-policy.md) の拡張項目。Collab の production-definition を土台にする
- それまでの間: カタログ上は対象外。顧客には既存給与／会計 SaaS＋エクスポートを推奨
- **作らない**: P09/P14 への給与ロジック混入。規制名乗り前にカタログへ載せない
...
- [21-talent-staging.md](./21-talent-staging.md)
- [22-attendance-staging.md](./22-attendance-staging.md)
- [23-media-staging.md](./23-media-staging.md)
- [./payroll-platform/payroll-tax-policy.md](./payroll-platform/payroll-tax-policy.md)
- [11-implementation-backlog-by-pxx.md](./11-implementation-backlog-by-pxx.md)
