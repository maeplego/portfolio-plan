...
Cursor ルール（`.cursor/rules/`）と Skills（`.cursor/skills/`）はこのファイルの運用要約である。矛盾したら **本ファイルを正** とする。

「P01 を実装して」のようにプロジェクトを指定されたら、**全部を先読みしない。** skill `start-pxx` に従い、次だけ読む。

1. 対象プロジェクトの `AGENTS.md`（なければ `instructions.md`）— 索引
2. `portfolio-plan/<project>/DESIGN.md` の**該当節**（必要なら overview / 元アイデアの該当節）
3. `chat-context/CURRENT.md`（あれば）。過去の `Pxx_*.md` は全読しない（不明点だけ Grep）
4. 工程・コミット・K8s などの詳細が必要なときだけ、本ファイルの該当節
...
各プロジェクトの `chat-context/` は gitignore 済み。エージェントは **全ログを読まない**（skill `record-chat-context`）。

### CURRENT.md（推奨・上書き可）

エージェントが次セッションで読む引き継ぎ。作業のたびに更新する。

- パス: `<project>/chat-context/CURRENT.md`
- 含めるもの: いまの目標、直近の決定、未決、次アクション、触った `pf-*` パス
- 長くしない（目安 40 行以内）。詳細は連番ログかコードへ

### 連番ログ（アーカイブ）

意味のある区切り（大きな方針決定・マイルストーン）で **新規ファイル 1 つ**。毎往復必須ではない。

- 既存ファイルを追記して履歴を混ぜない
- ファイル名: `{ID}_{5桁連番}_{内容の要約}.md`
  - 例: `P01_00001_engineering-standards-and-start.md`
  - 横断も **`Pxx_` 接頭**（例: `P16_…`）。旧 `MN_` は使わない
  - 連番は既存の最大 + 1。欠番を埋めない
- 含めるもの: 日時、依頼要約、決定、結果、次に残っていること
- 秘密は書かない。必要なら「環境変数 `X` を設定した」とだけ書く

任意: 採用した決定だけを `chat-context/DECISIONS.md` に追記してよい（エージェントは CURRENT の次にこれを優先してよい）。
