...
- 購入者は自分の注文だけ読める
- 単体 Compose でデモできる（Postgres + API + storefront）

### 含まない（意図的。計画スライスでも「できた」扱いにしない）

| 項目 | 理由 |
| --- | --- |
| 8 サービスへの分割 / overlay D / K8s | DESIGN: Compose のあと。本スライスはモノリス |
...
- 負在庫は拒否
- 一人開発。モジュールは同じ Go プロセス。テーブル接頭辞で境界を残す
...
| NFR-02 | Compose は専用 Postgres。K8s は出さない | 分割と基盤を同時に始めない |
...
6. `docker compose up` で storefront の `/demo` が同じ物語を見せる
