...
この段階の価値は **CSV 取り込み → デモ明細プレビュー → モック export** であり、画面量・クライアント状態は小さい。API と同一プロセスで `GET /` に HTML を返す方が Compose / overlay の入口が単純で、ビルド単位も増えない。

React を使わないのは技術的制約ではなく **デモ段階の薄いスライス** の選択。将来フル画面にするなら別アプリ（例: Next）でもよいが、当面は非目標。

手組み HTML とページ内スクリプトの `innerHTML` 更新があるため、表示値は `esc()` でエスケープする（Hono 自体が自動エスケープするわけではない）。
## 実装順
