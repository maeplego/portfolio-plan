...
- presign PUT、complete、クォータ、フォルダ、期限付き共有リンク
...
| 共有リンクのゲスト | トークンで 1 ファイルを取る人 | 共有トークン |
...
- key は `user/{sub}/ulid`
- `purpose` は `wiki` / `product` / `blog` / `chat` / `drive`
- 上限の例: 20MB、4000px。巨大画像は 413
...
| FR-08 | 共有リンクは高エントロピートークン。期限切れは 4xx | 推測と放置リンク |
