...
「P06 を実装して」と言われたら skill `start-pxx`。全部先読みせず、必要なものだけ:
...
9. `commerce-platform/chat-context/CURRENT.md`（あれば）。過去の `P06_*.md` は全読しない（不明点だけ Grep）
...
状態の引き継ぎは `chat-context/CURRENT.md` を更新（skill `record-chat-context`）。大きな区切りだけ `chat-context/P06_XXXXX_要約.md`（既存最大連番 + 1）。
